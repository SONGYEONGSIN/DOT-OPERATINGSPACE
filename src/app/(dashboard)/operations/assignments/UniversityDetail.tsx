...
function NameBtn({
  ...
  if (empty)
    return <span className="text-[11px] text-[var(--color-text-muted)]/30">—</span>;
  return (
    <button
      type="button"
      onClick={() => onClick(name!)}
      className="text-[11px] font-bold text-[var(--color-text)] hover:text-primary transition-colors"
    >
      {name}
    </button>
  );
...
export default function UniversityDetail({
  ...
  return (
    <div className="p-6 space-y-4">
      {/* Header */}
      <div className="flex items-center gap-3 flex-wrap border-b border-black/[0.04]/10 pb-4">
        <h2 className="text-lg font-black text-[var(--color-text)]">
          {u.universityName}
        </h2>
        <div className="flex items-center gap-2 text-[10px]">
          {u.category && (
            <span className="font-bold px-1.5 py-0.5 rounded bg-[var(--color-surface)] text-[var(--color-text-muted)]">
              {u.category}
            </span>
          )}
          {u.region && (
        ...
        {sections.map((sec) => {
          ...
          return (
            <div
              key={sec.label}
              className={cn(
                "rounded-[14px] border-l-[3px] overflow-hidden",
              ...
              <div className="grid grid-cols-2 px-3">
                ...
                <div className="pb-3">
                  ...
                  {sec.left.map((f) => (
                    <div
                      key={f.label}
                      className="flex items-center justify-between py-1.5 border-b border-black/[0.04]/5 last:border-b-0"
                    >
                ...
                {/* Right column */}
                <div className="pb-3 pl-4 border-l border-black/[0.04]/5">
                  <div className="py-1.5 mb-0.5">
                    <span className="text-[9px] font-extrabold uppercase tracking-widest text-primary bg-primary/10 dark:bg-primary/15 px-2 py-1 rounded">
                      {sec.rightLabel}
                    </span>
      ...
      {/* Remark */}
      {u.remark && (
        <div className="rounded-[14px] border-l-[3px] border-l-outline-variant/40 bg-[var(--color-surface)]/30 p-3">
          <span className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-[var(--color-text-muted)]">
            비고
          </span>
          <p className="text-xs text-[var(--color-text-muted)] mt-1">{u.remark}</p>
        </div>
      )}
    </div>
  );
