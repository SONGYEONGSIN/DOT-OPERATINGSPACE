...
function SectionHeader({
  ...
  return (
    <div className="flex items-center gap-2 mb-4">
      ...
      <h3 className="text-sm font-bold text-[var(--color-text)] tracking-wide">
        {title}
      </h3>
    </div>
  );
...
function HBar({
  ...
  return (
    <div className="flex items-center gap-3 group">
      {rank !== undefined && (
        <span className="text-[11px] font-bold text-[var(--color-text-muted)] w-4 text-right tabular-nums shrink-0">
          {rank}
        </span>
      )}
      <span
        className="text-xs text-[var(--color-text)] w-28 shrink-0 truncate"
        title={label}
      >
        {label}
      </span>
      <div className="flex-1 h-5 bg-black/[0.06] rounded-full overflow-hidden">
        <div
          className="h-full bg-[var(--color-primary)] rounded-full transition-all duration-500"
          style={{ width: `${Math.max(pct, 2)}%` }}
        />
      </div>
      <span className="text-xs font-bold text-[var(--color-text)] tabular-nums w-10 text-right shrink-0">
        {count}
      </span>
    </div>
  );
        ...
        {segments.map((seg) => {
          ...
          return (
            <div key={seg.label} className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-[var(--color-text)] tabular-nums ml-auto">
                {seg.count}
              </span>
              <span className="text-[11px] text-[var(--color-text-muted)] tabular-nums w-12 text-right">
                {pct}%
              </span>
            </div>
          );
...
export default function IncidentAnalytics({
  ...
  const categoryStats = useMemo(
    () => countBy(incidents, (i) => i.category),
    [incidents],
  );
  ...
  const assigneeStats = useMemo(
    () => countBy(incidents, (i) => i.assignee).slice(0, 15),
    [incidents],
  );
  ...
  const uniStats = useMemo(
    () => countBy(incidents, (i) => i.university).slice(0, 15),
    [incidents],
  );
  ...
  const statusSegments = useMemo(() => {
    const order = [
      { label: "처리완료", color: "var(--color-chart-primary)" },
      { label: "할 일", color: "var(--color-chart-warning)" },
      { label: "진행 중", color: "var(--color-chart-blue)" },
      { label: "보류", color: "var(--color-chart-danger)" },
      { label: "처리예정", color: "var(--color-chart-purple)" },
    ];
    const map = new Map<string, number>();
    for (const inc of incidents) {
    ...
    for (const [label, count] of map.entries()) {
      if (!order.some((o) => o.label === label)) {
        segments.push({ label, count, color: "var(--color-chart-muted)" });
      }
    }
    return segments;
  }, [incidents]);
  ...
  const keywords = useMemo(() => {
    ...
    for (const inc of incidents) {
      ...
      const text = [inc.background, inc.cause, inc.resolution]
        .filter(Boolean)
        .join(" ");
      if (!text) continue;
      const tokens = text
    ...
    <div className="space-y-6">
      ...
      <Card className="p-5">
        ...
        <div className="overflow-x-auto">
          <div
            className="flex items-end gap-1.5 min-w-max"
            style={{ height: 180 }}
          >
            {monthlyStats.map((m) => {
              ...
              return (
                <div key={m.month} className="flex flex-col items-center gap-1">
                  ...
                  <div
                    className="w-8 bg-[var(--color-primary)] rounded-t-[8px] transition-all duration-500"
                    style={{ height: Math.max(h, 4) }}
                  />
                  <span className="text-[10px] text-[var(--color-text-muted)] tabular-nums whitespace-nowrap -rotate-45 origin-top-left translate-y-1">
                    {m.month.slice(2)}
                  </span>
                </div>
              );
        ...
        <Card className="p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {deptStats.map((d) => (
              <div
                key={d.key}
                className="flex flex-col items-center gap-1 bg-[var(--color-surface)] shadow-neu-soft rounded-[14px] py-5 px-3"
              >
          ...
          {keywords.length > 0 ? (
            <div className="flex flex-wrap gap-2 items-center">
              {keywords.map((kw) => {
                ...
                const ratio =
                  kwMax === kwMin ? 0.5 : (kw.count - kwMin) / (kwMax - kwMin);
                const fontSize = 11 + ratio * 13;
                const opacity = 0.5 + ratio * 0.5;
                return (
                  <span
                    key={kw.word}
                    className="text-[var(--color-primary)] font-bold cursor-default transition-opacity hover:opacity-100"
