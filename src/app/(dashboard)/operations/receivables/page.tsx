...
export default async function ReceivablesPage({ searchParams }: PageProps) {
  ...
  const tableData = filteredItems.map((item) => {
    ...
    return {
      invoiceDate: (
        <span className="text-xs text-[var(--color-text)] tabular-nums">{item.invoiceDate ?? "-"}</span>
      ),
      university: (
        <span className="text-sm font-medium text-[var(--color-text)]">{item.university ?? "-"}</span>
      ),
      detail: (
        <span className="text-xs text-[var(--color-text-muted)] truncate block max-w-[200px]">
          {item.detail ? stripNumbers(item.detail) : "-"}
        </span>
      ),
      operator: (
        <span className="text-xs text-[var(--color-text)]">{item.operator ?? "-"}</span>
      ),
      amount: (
      ...
      schoolContact: (
        <span className="text-xs text-[var(--color-text-muted)]">{item.schoolContact ?? "-"}</span>
      ),
      mailSentDate: (
        <span className="text-xs text-[var(--color-text-muted)] tabular-nums">{item.mailSentDate ?? "-"}</span>
      ),
      expectedPayDate: (
        <span className="text-xs text-[var(--color-text-muted)] tabular-nums">{item.expectedPayDate ?? "-"}</span>
      ),
      daysElapsed: (
        <span className={`text-xs font-bold tabular-nums ${item.daysElapsed > 60 ? "text-error" : item.daysElapsed > 30 ? "text-tertiary" : "text-[var(--color-text)]"}`}>
          {item.daysElapsed}일
        </span>
      ),
      payStatus: hasMemo ? (
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-xs text-[var(--color-text-muted)]">
        기준일: <span className="font-bold text-[var(--color-text)]">{sheetName}</span> (SharePoint 연동)
      </div>

      <KpiGrid>
        <KpiCard
          icon={<IconWallet size={18} className="text-[var(--color-text-muted)]" />}
          label="청구금액"
          value={totalAmount.toLocaleString()}
          suffix="원"
        />
        <KpiCard
          icon={<IconCash size={18} className="text-[var(--color-text-muted)]" />}
          label="입금금액"
      ...
      <Card className="p-5">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-bold text-[var(--color-text)]">회수율</h2>
        ...
        <div className="flex items-center justify-between mt-3 text-xs text-[var(--color-text-muted)]">
          <span>입금 <span className="font-bold text-[var(--color-text)]">{paidAmount.toLocaleString()}원</span> ({paidItems.length}건)</span>
