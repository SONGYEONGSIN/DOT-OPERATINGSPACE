  ...
  return (
    <>
      <div ref={menuRef} className="relative">
        ...
        {isOpen && (
          <div className="absolute right-0 top-full mt-1 z-10 min-w-[130px] bg-[var(--color-surface)] rounded-[14px] border border-black/[0.04]/15 shadow-neu-strong">
            <button type="button" onClick={() => { setIsOpen(false); setShowEdit(true); setError(""); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-[var(--color-text)] hover:bg-surface-bright transition-colors rounded-t-lg">
              <IconEdit size={16} className="text-[var(--color-text-muted)]" />
              수정
            </button>
            <button type="button" onClick={handleUploadClick} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-[var(--color-text)] hover:bg-surface-bright transition-colors rounded-b-lg">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-md bg-[var(--color-surface)] rounded-[20px] border border-black/[0.04]/15 shadow-neu-strong animate-slide-up overflow-visible">
            <div className="flex items-center justify-between px-6 py-4 border-b border-black/[0.04]/10">
              <h2 className="text-lg font-bold text-[var(--color-text)]">공문 수정</h2>
              ...
              <div className="p-6 space-y-4 overflow-visible">
                ...
                <div><label className={labelClass}>파일 링크</label>
                  <div className="flex items-center gap-2">
                    ...
                    {editFileLink && (
                      <a href={editFileLink.startsWith("http") ? editFileLink : "#"} target="_blank" rel="noopener noreferrer" className="shrink-0 p-2 rounded-[14px] bg-primary/10 text-primary hover:bg-primary/20 transition-colors">
                        <IconFile size={16} />
                      </a>
                    )}
                  </div>
