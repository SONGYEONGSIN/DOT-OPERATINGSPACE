...
export default function EditTaskModal({ open, onClose, task, profiles }: EditTaskModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md bg-surface-container rounded-2xl border border-outline-variant/15 shadow-2xl animate-slide-up overflow-visible max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-outline-variant/10">
          <div>
            <h2 className="text-lg font-bold text-on-surface">작업 수정</h2>
            {task.requester && (
              <p className="text-[10px] text-on-surface-variant mt-0.5">요청자: {task.requester}</p>
            )}
          </div>
        ...
        {success ? (
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-visible">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[10px] font-bold text-on-surface-variant tracking-widest uppercase mb-2">단계</label>
