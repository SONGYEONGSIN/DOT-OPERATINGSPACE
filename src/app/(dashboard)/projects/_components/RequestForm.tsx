...
export default function RequestForm({ project, year, profiles }: RequestFormProps) {
  ...
  if (success) {
    return (
      <Card className="p-12 text-center">
        ...
        <p className="text-xs text-[var(--color-text-muted)] mt-1">담당자가 확인 후 처리합니다.</p>
      </Card>
    );
  }
  ...
  return (
    <Card className="p-6">
      ...
      <p className="text-xs text-[var(--color-text-muted)] mb-6">프로젝트 담당자에게 개발/수정 요청을 등록합니다.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-2">
            {profiles.map((p) => (
              <button
                ...
                className={cn(
                  "flex items-center gap-2 p-2.5 rounded-[14px] border-2 transition-all text-left",
                  requester === p.name
        ...
        <button
          ...
          className={cn(
            "w-full flex items-center justify-center gap-2 py-3 bg-primary text-on-primary font-bold rounded-[14px] active:scale-95 transition-transform text-sm",
